...
- **IPC도 검색식에 결합된다.** 1.0은 발명 자료에서 IPC를 가져오지만 5단계는
  학생 자기 아이디어라 줄 곳이 없었다. `generate_kipris_query` 에 `ipc` 를 열어
  AI가 고르되, **`ipc_descriptions` 표에 있는 코드만 통과**시킨다. 없는 코드는
  버리고 무엇을 버렸는지 AI에게 알려 준다 (원칙 4의 연장 — 지어낸 값은 안 쓴다).
...
- ✅ **5단계 검색식에 IPC 결합** — `IPC=[A45B]*(우산+우산 커버+우산 덮개)*(흡수+흡수재+빗물 흡수)`
  356건. 이때 모달 안 특허 검색을 해도 이 5단계 기록은 그대로 남는 것까지 확인
...
- **로컬 커밋은 자유, push·배포는 반드시 승인 후.** 지금까지 커밋 18개, push 이력 없음.
