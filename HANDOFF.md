...
### ⚠️ 대표님이 해 주셔야 하는 것 (관리자 페이지가 저장되려면)

1. **Supabase에서 SQL 한 번 실행** — `supabase/prompt_overrides.sql` 을
   Supabase 대시보드 → SQL Editor 에 붙여넣고 Run.
   이걸 안 하면 /admin 에서 글을 볼 수는 있어도 **저장이 안 된다**
   (화면에 "표가 없습니다"라고 안내가 뜬다).
2. **`.env.local` 의 `ADMIN_PASSWORD` 를 원하는 값으로 바꾸기** —
   지금은 확인용으로 `ideaprism-admin` 이 들어가 있다. 배포한다면 Vercel
   환경변수에도 같은 이름으로 넣어야 한다.

확인은 http://localhost:3000/api/health 의 `promptOverrides` 항목으로 한다.
...
### 이어서 손본 것 (2026-08-11, 커밋 `ffcd2e1`)

대표님 사용 소감 4건을 반영했다.

- **말이 끝나면 커서가 입력창으로 돌아온다.** 말하는 동안 입력창이 잠기는데,
  잠기는 순간 브라우저가 포커스를 가져가 매번 마우스로 다시 눌러야 했다.
- **진행판을 눌러 앞 단계로 돌아간다.** `quest.furthestStage`(가 본 가장 먼 단계)와
  `quest.history`(고치기 전 기록)를 더했다.
  - 지나온 단계만 눌린다. **안 가 본 단계로 건너뛰는 것은 여전히 막는다**(원칙 2).
  - 돌아가면 그 단계 캐릭터가 자기소개 없이 맞아 준다 (`intent: "revisit"`).
  - 다시 정리하면 최종본이 갱신되고 정상대로 다음 단계로 오른다. 가 있던 자리는
    남아 있어 진행판에서 언제든 복귀한다.
  - **최종본이 발명노트 본문, 밀려난 것은 아래 접이식 '고치기 전 기록'** (대표님 방침:
    "최종을 기준으로 기록해. 시행착오도 의미가 있으니 참고로써 기록").
- **0~4단계는 학생이 우측 창을 직접 조작한다.** 선배가 창만 띄우고 계속 대화로만
  풀던 문제. 원인은 **학생이 만질 수단 자체가 없었던 것** — 검색은 AI만 할 수 있었다.
  검색 패널에 학생용 검색창을 넣고 `/api/search` 를 신설했다. 학생이 찾은 결과도
  세션에 실려 다음 턴에 선배가 같은 화면을 본다.
  `flow`: 처음 한 번만 띄워 주고, 그다음엔 **대신 누르지 말고 조작법을 알려 준 뒤
  학생에게 넘긴다. 막혔다고 할 때만 거든다.**
- **탐정은 "이렇다는 거지 → 이렇게 찾아볼게" 하고 찾는다.** `flow/5` 에만 적었더니
  안 지켜져서(도구를 몰아 부르고 결과만 통보) **`flow/공통규칙.md` 로 올렸다** —
  공통규칙은 시스템 프롬프트에 들어가 훨씬 강하게 걸린다.
  조작 주도권은 5단계 탐정만 (앞 단계는 학생이 쥔다).

### 관리자 페이지 (2026-08-11, 커밋 `c6a2f3f`)

`/admin` 에서 캐릭터 대본 3편 + 대화 흐름 9편을 골라 고친다.

- **저장은 파일이 아니라 Supabase로 간다.** Vercel에 올리면 서버의 파일이
  읽기 전용이라 파일 쓰기가 배포 환경에서 아예 안 되고, 파일을 덮어쓰면
  원본이 사라져 되돌릴 수 없다. 그래서 두 겹이다:
  **파일 = 공장 초기값(git에 그대로), `prompt_overrides` 표 = 고친 값(있으면 우선).**
  "처음으로" 버튼은 표에서 한 줄을 지우는 일이라 언제나 안전하게 되돌아간다.
- **잠금이 기본값이다.** `ADMIN_PASSWORD` 환경변수가 없으면 관리자 페이지가
  아예 열리지 않는다. 실수로 잠금 없이 배포되는 일을 막기 위해서다.
  로그인하면 서명 쿠키를 주고, 코드 자체는 쿠키에 담지 않는다.
- **"AI가 받는 내용" 토글** — 안내 주석과 페르소나의 이미지 지시문이 걸러진 뒤의
  글을 미리 본다. 알맹이가 비면 저장을 막는다(캐릭터가 통째로 사라지는 것 방지).
- 고친 값은 20초쯤 캐시된다. 저장 직후에는 캐시를 비우므로 바로 반영되지만,
  배포 환경은 서버가 여러 대라 다른 서버는 최대 그만큼 옛 값을 쓸 수 있다.
- `personas.ts` 는 순수 함수(`sanitizePersona`)만 남기고 읽기는
  `prompts/persona.ts` 로 옮겼다 — **회귀 테스트가 `personas.ts` 를 불러 쓰는데
  `server-only` 를 들이면 테스트가 깨진다.** 이 분리를 되돌리지 말 것.

### 점검 화면을 믿을 때 주의 (실측으로 겪음)

Supabase에서 `head: true` 로 **개수만 세는 조회는 표가 아예 없어도 오류를 돌려주지
않는다.** 그래서 `/api/health` 가 없는 표를 두고 "준비됨"이라고 초록으로 보고했다.
지금은 행을 실제로 받아 보게 고쳤다. 새 점검 항목을 만들 때 같은 함정에 빠지지 말 것.
...
- `POST /api/search` → 학생이 우측 패널에서 직접 검색 (AI 호출 없음)
- `/api/admin/session` → 관리자 로그인·로그아웃·상태
- `/api/admin/prompts`, `/api/admin/prompts/[kind]/[name]` → 글 목록·읽기·저장·되돌리기
...
- 필터를 바꾸면 세션에 실려 **다음 턴에 AI도 같은 화면을 본다** (PRD S-4).
  학생이 직접 검색한 것도 마찬가지다 — 서버 캐시에도 넣어 두므로 AI가 통계를
  물어봐도 학생이 방금 본 것과 같은 결과셋으로 답한다.
...
```
personas/     캐릭터 대본 — "어떻게 말하는가"    (공장 초기값. /admin 에서도 고침)
flow/         대화 흐름 지침 — "어떻게 흘러가는가" (공장 초기값, 9개 파일)
supabase/     invention_notes DDL(적용됨) · prompt_overrides DDL(대표님 실행 대기)
tests/        회귀 테스트 87개
src/app/admin/  관리자 화면 (프롬프트 편집)
src/lib/
  admin/      관리자 잠금 (접근 코드·서명 쿠키)
  prompts/    프롬프트 저장소 (파일 기본값 + DB 덮어쓰기) · 페르소나 읽기
...
- **로컬 커밋은 자유, push·배포는 반드시 승인 후.** 지금까지 커밋 24개, push 이력 없음.
